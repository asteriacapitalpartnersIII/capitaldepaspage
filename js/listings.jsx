...
const PropertyCard = ({ prop, setPage, setSelectedProperty }) => {
    ...
    const heroImg = (prop.photos && prop.photos[0]) || PROJECT_IMAGES[prop.slug] || PROJECT_IMAGES[prop.id] || null;
    return (
    <div ref={cardRef} onClick={handleClick} onMouseMove={onMouseMove}
        ...
        <div style={{
            ...
            background: heroImg
                ? 'transparent'
                : `linear-gradient(160deg,hsl(${prop.accentHue},55%,88%),hsl(${prop.accentHue},40%,78%))`
        }}>
            {heroImg && (
                <img
                    src={heroImg}
                    alt={prop.name}
                    onError={e=>{ e.target.style.display='none'; e.target.parentNode.style.background=`linear-gradient(160deg,hsl(${prop.accentHue},55%,88%),hsl(${prop.accentHue},40%,78%))`; }}
                    style={{
            ...
            {!heroImg && Array.from({length:5}).map((_,r)=>Array.from({length:7}).map((_,c)=>(
                <div key={`${r}${c}`} style={{
                    position:'absolute', width:10, height:14, borderRadius:2,
                    background:`hsla(${prop.accentHue},50%,25%,${Math.random()>0.4?0.35:0.07})`,
                    left:14+c*20, top:16+r*30
                }} />
            )))}
            <div style={{
                position:'absolute', top:14, left:14,
                background:prop.type==='preventa'?'#1550E8':'#16a34a',
                color:'#fff', fontSize:10, fontFamily:'DM Sans', fontWeight:700,
                letterSpacing:1.5, textTransform:'uppercase', padding:'4px 10px', borderRadius:100
            }}>{prop.type}</div>
            <div style={{
...
const Listings = ({ setPage, setSelectedProperty }) => {
    ...
    const availableBuckets = React.useMemo(() => {
        const seen = new Set();
        PROPERTIES.forEach(p => { const b = zoneToFilter(p.zone); if (b) seen.add(b); });
        return seen;
    }, [PROPERTIES.length]);

    const zones = React.useMemo(() => {
        const chips = [{ id:'todos', label:'Todos' }];
        ['cdmx','cancun','cabos','guadalajara','monterrey','oaxaca','acapulco','vallarta']
            .filter(b => availableBuckets.has(b))
            .forEach(b => chips.push({ id:b, label: BUCKET_LABELS[b] || b }));
        return chips;
    }, [availableBuckets]);

    React.useEffect(() => {
        if (filter !== 'todos' && !availableBuckets.has(filter)) setFilter('todos');
    }, [availableBuckets]);

    let filtered = PROPERTIES.filter(p => {
        if (filter !== 'todos' && zoneToFilter(p.zone) !== filter) return false;
    ...
    const chipStyle = (active) => ({
        ...
        background:active?'#1550E8':'#fff',
        border:`1.5px solid ${active?'#1550E8':'rgba(0,0,0,0.1)'}`,
        borderRadius:100, padding:'7px 18px', cursor:'pointer', transition:'all 0.2s',
        whiteSpace:'nowrap', boxShadow:active?'0 4px 12px rgba(21,80,232,0.25)':'none',
    });

    return (
        <div style={{ minHeight:'100vh', background:'#F5F3EE', padding:'100px 20px 24px' }}>
            <div ref={ref} style={{ opacity:vis?1:0, transform:vis?'none':'translateY(30px)', transition:'all 0.8s ease' }}>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:10, marginBottom:28, alignItems:'center' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10, background:'#fff', border:'1.5px solid rgba(0,0,0,0.08)', borderRadius:100, padding:'7px 18px', minWidth:250 }}>
                        ...
                        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar proyecto o colonia..." style={{ background:'none', border:'none', outline:'none', fontFamily:'DM Sans', fontSize:13, color:'#0E0E0C', width:'100%', caretColor:'#1550E8' }} />
                    </div>
...
Object.assign(window, { Listings, PropertyCard, PROJECT_IMAGES });
