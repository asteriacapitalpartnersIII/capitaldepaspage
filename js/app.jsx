...
const TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{ "heroVariant": "cinematic", "accentColor": "#1550E8", "showWhatsApp": true, "showFeatured": true }/*EDITMODE-END*/;
...
function readRouteFromLocation() {
  try {
    ...
    if (path === '/calculadora') return { page: 'calculator', slug: null };
    const m = path.match(/^\/proyecto\/([^\/]+)$/);
    if (m) return { page: 'project', slug: decodeURIComponent(m[1]) };
    return { page: 'home', slug: null };
  } catch (e) { return { page: 'home', slug: null }; }
}
...
const App = () => {
  ...
  React.useEffect(() => {
    ...
    try { window.capdepasTrack && window.capdepasTrack('page_view', { page }); } catch(e){}
  }, [page]);
  ...
  React.useEffect(() => {
    ...
    if (match) { setSelectedProperty(match); setPage('project'); setPendingSlug(null); }
  ...
  React.useEffect(() => {
    const onPop = () => {
      ...
      if (r.page === 'project' && r.slug) {
        ...
        if (match) { setSelectedProperty(match); setPage('project'); setPendingSlug(null); }
        else { setPendingSlug(r.slug); setPage('project'); }
      } else { setPendingSlug(null); setPage(r.page); }
    };
    ...
    <section style={{ background:'#F5F3EE', padding:'100px 60px' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:48, flexWrap:'wrap', gap:16 }}>
        <div>
          ...
          <h2 style={{ fontFamily:'Playfair Display', fontSize:'clamp(28px,5vw,48px)', fontWeight:700, color:'#0E0E0C', margin:0, letterSpacing:-1 }}>Proyectos <span style={{ color:'#1550E8' }}>destacados</span></h2>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:24 }}>
        {PROPERTIES.slice(0,3).map(p=>(<PropertyCard key={p.id} prop={p} setPage={goTo} setSelectedProperty={selectProperty}/>))}
      </div>
    </section>
  );

  const CalculatorPage = () => (
    <div style={{ paddingTop:80, background:'#F5F3EE', minHeight:'100vh' }}>
      <div style={{ textAlign:'center', padding:'60px 24px 0' }}>
        <div style={{ fontFamily:'DM Sans', fontSize:12, letterSpacing:4, color:'rgba(21,80,232,0.6)', textTransform:'uppercase', marginBottom:12 }}>Herramienta gratuita</div>
        <h1 style={{ fontFamily:'Playfair Display', fontSize:'clamp(32px,5vw,56px)', fontWeight:700, color:'#0E0E0C', margin:'0 0 16px', letterSpacing:-1 }}>Calculadora de crédito <span style={{ color:'#1550E8' }}>hipotecario</span></h1>
        <p style={{ fontFamily:'DM Sans', fontSize:16, color:'#6B6560', maxWidth:520, margin:'0 auto 0' }}>Estima tu mensualidad según precio, enganche y plazo. Compatible con crédito bancario, Infonavit y Fovissste.</p>
      </div>
      <Calculator/>
    </div>
  );
  ...
  return (
    <div style={{ background:'#F5F3EE', minHeight:'100vh', cursor:'none' }}>
      ...
      {page==='calculator' && <><CalculatorPage/><Footer setPage={goTo}/></>}
