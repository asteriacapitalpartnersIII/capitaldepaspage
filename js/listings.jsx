...
const PropertyCard = ({ prop, setPage, setSelectedProperty }) => { const cardRef = React.useRef(null); const [tilt, setTilt] = React.useState({ x:0, y:0 }); const [hovered, setHovered] = React.useState(false); const onMouseMove = (e) => { const r = cardRef.current.getBoundingClientRect(); const x=(e.clientX-r.left)/r.width, y=(e.clientY-r.top)/r.height; setTilt({ x:(y-0.5)*10, y:(x-0.5)*-10 }); }; const handleClick = () => { setSelectedProperty(prop); setPage('project'); window.scrollTo(0,0); }; return ( <div ref={cardRef} onClick={handleClick} onMouseMove={onMouseMove} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>{setHovered(false);setTilt({x:0,y:0});}} data-cursor="property" style={{ background:'#fff', border:`1.5px solid ${hovered?'rgba(21,80,232,0.3)':'rgba(0,0,0,0.06)'}`, borderRadius:20, overflow:'hidden', cursor:'pointer', boxShadow: hovered?'0 20px 60px rgba(21,80,232,0.12), 0 4px 20px rgba(0,0,0,0.06)':'0 2px 12px rgba(0,0,0,0.06)', transform:`perspective(1000px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg) scale(${hovered?1.02:1})`, transformStyle:'preserve-3d', transitionProperty:'transform,border-color,box-shadow', transitionDuration:hovered?'0.05s,0.3s,0.3s':'0.5s,0.3s,0.3s', }}>
        ...
        <div style={{ padding:'18px 20px 20px' }}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}><div><div style={{ fontFamily:'DM Sans', fontSize:15, fontWeight:700, color:'#0E0E0C' }}>{prop.name}</div><div style={{ fontFamily:'DM Sans', fontSize:12, color:'#9E9890', marginTop:3, display:'flex', alignItems:'center', gap:5 }}><svg width="10" height="12" viewBox="0 0 24 28" fill="#1550E8" opacity="0.5"><path d="M12 0C7.6 0 4 3.6 4 8c0 6 8 18 8 18s8-12 8-18c0-4.4-3.6-8-8-8zm0 11a3 3 0 110-6 3 3 0 010 6z"/></svg>{prop.location}</div></div><div style={{ fontFamily:'DM Sans', fontSize:20, fontWeight:800, color:'#1550E8' }}>{prop.priceStr}</div></div>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}><div style={{ fontFamily:'DM Sans', fontSize:11, color:'#9E9890' }}>Entrega: <span style={{color:'#1550E8',fontWeight:600}}>{prop.completion}</span></div><div style={{ fontFamily:'DM Sans', fontSize:11, color:'#9E9890' }}>{prop.developer}</div></div>
        </div></div> ); };
...
const Listings = ({ setPage, setSelectedProperty }) => {
    ...
    return ( <div style={{ minHeight:'100vh', background:'#F5F3EE', padding:'100px 20px 24px' }}> <div ref={ref} style={{ opacity:vis?1:0, transform:vis?'none':'translateY(30px)', transition:'all 0.8s ease' }}>
          <div style={{ marginBottom:44 }}><div style={{ fontFamily:'DM Sans', fontSize:12, letterSpacing:4, color:'rgba(21,80,232,0.6)', textTransform:'uppercase', marginBottom:10 }}>Marketplace</div><h2 style={{ fontFamily:'Playfair Display', fontSize:52, fontWeight:700, color:'#0E0E0C', margin:0, letterSpacing:-1 }}>Encuentra tu <span style={{ color:'#1550E8' }}>departamento</span></h2><p style={{ fontFamily:'DM Sans', fontSize:16, color:'#9E9890', marginTop:10 }}>{PROPERTIES.length} proyectos disponibles en M\u00e9xico</p></div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:10, marginBottom:28, alignItems:'center' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10, background:'#fff', border:'1.5px solid rgba(0,0,0,0.08)', borderRadius:100, padding:'7px 18px', minWidth:250 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1550E8" strokeWidth="2" opacity="0.6"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar proyecto o colonia..." style={{ background:'none', border:'none', outline:'none', fontFamily:'DM Sans', fontSize:13, color:'#0E0E0C', width:'100%', caretColor:'#1550E8' }} /></div>
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>{zones.map(z=><button key={z.id} onClick={()=>setFilter(z.id)} style={chipStyle(filter===z.id)} data-cursor="pointer">{z.label}</button>)}</div>
                  <div style={{ display:'flex', gap:8, marginLeft:'auto' }}>{['todos','preventa','venta'].map(t=><button key={t} onClick={()=>setTypeFilter(t)} style={chipStyle(typeFilter===t)} data-cursor="pointer">{t==='todos'?'Todos':t.charAt(0).toUpperCase()+t.slice(1)}</button>)}</div>
                  <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{ background:'#fff', border:'1.5px solid rgba(0,0,0,0.1)', borderRadius:100, padding:'7px 18px', fontFamily:'DM Sans', fontSize:13, color:'#6B6560', cursor:'pointer', outline:'none' }}><option value="default">Ordenar</option><option value="price-asc">Precio: menor</option><option value="price-desc">Precio: mayor</option></select>
          </div>
