...
// ── TSV parser (fallback si la hoja tiene tabs como separador) ────────────────
function parseTSV(text) {
    return text
      .split('\n')
      .map(line => line.replace(/\r$/, '').split('\t').map(c => c.trim()))
      .filter(row => row.some(c => c));
}

// Detecta si los datos fueron exportados como una sola columna con tabs embebidos
// (ocurre cuando el sheet tiene datos pegados en una sola celda como TSV)
function detectAndParse(text) {
    const csvRows = parseCSV(text);
    if (csvRows.length === 0) return csvRows;

  // Si la primera fila tiene más de 2 columnas, el CSV es correcto
  if (csvRows[0].length > 2) return csvRows;

  // Si la primera celda contiene tabs, re-parsear como TSV
  const firstCell = csvRows[0][0] || '';
    if (firstCell.includes('\t')) {
          // El CSV tiene tabs dentro de celdas — necesitamos parsear cada celda como TSV
      const result = [];
          for (const csvRow of csvRows) {
                  // Cada "celda" es realmente una fila TSV completa
            const tsvRow = csvRow[0].split('\t').map(c => c.trim());
                  if (tsvRow.some(c => c)) result.push(tsvRow);
          }
          return result;
    }
...
async function fetchSheet(sheetName) {
    try {
          ...
          const rows = detectAndParse(text);
