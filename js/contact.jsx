// ── capitaldepas.com · Contact + FAQ + Privacy (Light Theme, Mobile Responsive) ──────────────────────────
const Contact = () => {
  ...
  const submit = (e) => {
    e.preventDefault();
    if(form.privacy) (function(){try{window.capdepasTrack && window.capdepasTrack("lead", { source:"contact_form" });}catch(e){}})();
    setSent(true);
  };
  const inp = { width:'100%', background:'#fff', border:'1.5px solid rgba(0,0,0,0.1)', borderRadius:12, padding:'13px 16px', fontFamily:'DM Sans', fontSize:14, color:'#0E0E0C', outline:'none', caretColor:'#1550E8', boxSizing:'border-box', transition:'border-color 0.2s' };
  return (
    <section style={{ background:'#F5F3EE', padding:'80px 24px', position:'relative' }}>
      {/* Privacy modal */}
      {showPrivacy && (
        <div style={{ position:'fixed', inset:0, zIndex:9000, background:'rgba(0,0,0,0.5)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }} onClick={()=>setShowPrivacy(false)}>
          <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', border:'1.5px solid rgba(0,0,0,0.08)', borderRadius:24, padding:'32px 24px', maxWidth:580, maxHeight:'80vh', overflowY:'auto', position:'relative', width:'100%' }}>
      ...
      <div ref={ref} style={{ opacity:vis?1:0, transform:vis?'none':'translateY(40px)', transition:'all 0.9s ease', maxWidth:1000, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:48 }}>
          ...
          <h2 style={{ fontFamily:'Playfair Display', fontSize:'clamp(28px,5vw,48px)', fontWeight:700, color:'#0E0E0C', margin:0, letterSpacing:-1 }}>Habla con un <span style={{ color:'#1550E8' }}>asesor</span></h2>
          <p style={{ fontFamily:'DM Sans', fontSize:16, color:'#9E9890', marginTop:12 }}>Sin compromisos. Te respondemos en menos de 24 horas.</p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:32, marginBottom:64 }}>
          {/* Form */}
          <div style={{ background:'#fff', borderRadius:24, padding:'28px 24px', boxShadow:'0 4px 30px rgba(0,0,0,0.06)', border:'1.5px solid rgba(0,0,0,0.06)' }}>
            {sent ? (
              ...
              <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                ...
                <input value={form.name} onChange={e=>update('name',e.target.value)} placeholder="Nombre completo" required style={inp} onFocus={e=>e.target.style.borderColor='#1550E8'} onBlur={e=>e.target.style.borderColor='rgba(0,0,0,0.1)'}/>
                <input value={form.phone} onChange={e=>update('phone',e.target.value)} placeholder="Teléfono" style={inp} onFocus={e=>e.target.style.borderColor='#1550E8'} onBlur={e=>e.target.style.borderColor='rgba(0,0,0,0.1)'}/>
                <input value={form.email} onChange={e=>update('email',e.target.value)} placeholder="Correo electrónico" type="email" required style={inp} onFocus={e=>e.target.style.borderColor='#1550E8'} onBlur={e=>e.target.style.borderColor='rgba(0,0,0,0.1)'}/>
                ...
                <textarea value={form.message} onChange={e=>update('message',e.target.value)} placeholder="Cuéntanos más sobre lo que buscas..." rows={4} style={{...inp, resize:'vertical', minHeight:96}} onFocus={e=>e.target.style.borderColor='#1550E8'} onBlur={e=>e.target.style.borderColor='rgba(0,0,0,0.1)'}/>
                <label style={{ display:'flex', alignItems:'flex-start', gap:10, cursor:'pointer' }}>
                  ...
                  <span style={{ fontFamily:'DM Sans', fontSize:12, color:'#9E9890', lineHeight:1.6 }}>He leído y acepto el{' '}<button type="button" onClick={()=>setShowPrivacy(true)} style={{ background:'none', border:'none', color:'#1550E8', cursor:'pointer', padding:0, fontSize:12, fontFamily:'DM Sans', fontWeight:600, textDecoration:'underline' }} data-cursor="pointer">Aviso de Privacidad</button></span>
                </label>
                <button type="submit" disabled={!form.privacy} style={{ background:form.privacy?'#1550E8':'rgba(0,0,0,0.06)', border:'none', borderRadius:12, padding:'14px', fontFamily:'DM Sans', fontSize:15, fontWeight:700, color:form.privacy?'#fff':'#C5C0B8', cursor:form.privacy?'pointer':'not-allowed', transition:'all 0.3s', boxShadow:form.privacy?'0 4px 16px rgba(21,80,232,0.3)':'none' }} data-cursor="pointer">Enviar mensaje</button>
              </form>
            )}
          </div>
          {/* Info panel */}
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            {waNumber && (
              <div style={{ background:'#fff', border:'1.5px solid rgba(37,211,102,0.25)', borderRadius:20, padding:'20px 22px', boxShadow:'0 4px 20px rgba(37,211,102,0.07)' }}>
                ...
                <a href={"https://wa.me/"+waNumber+"?text=Hola, quiero información sobre capitaldepas.com"} target="_blank" rel="noreferrer" style={{ display:'block', textAlign:'center', background:'rgba(37,211,102,0.1)', border:'1px solid rgba(37,211,102,0.25)', borderRadius:10, padding:'12px', fontFamily:'DM Sans', fontSize:14, fontWeight:700, color:'#16a34a', textDecoration:'none', transition:'all 0.2s' }} data-cursor="pointer">+{waNumber}</a>
              </div>
            )}
            <div style={{ background:'#fff', borderRadius:18, padding:'20px 22px', border:'1.5px solid rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                {[{n:'IG',c:'#E1306C',url:'https://instagram.com/capitaldepas'},{n:'FB',c:'#1877F2',url:'https://facebook.com'},{n:'TK',c:'#ff0050',url:'https://tiktok.com'},{n:'LI',c:'#0A66C2',url:'https://linkedin.com'}].map(s=>(
                  <a key={s.n} href={s.url} target="_blank" rel="noreferrer" style={{ flex:1, minWidth:56, height:40, borderRadius:10, background:`${s.c}12`, border:`1.5px solid ${s.c}25`, display:'flex', alignItems:'center', justifyContent:'center', textDecoration:'none', fontFamily:'DM Sans', fontSize:12, fontWeight:700, color:s.c, transition:'all 0.2s' }} data-cursor="pointer">{s.n}</a>
                ))}
              </div>
            </div>
        ...
        <div style={{ maxWidth:760, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:40 }}>
            <h3 style={{ fontFamily:'Playfair Display', fontSize:'clamp(24px,4vw,36px)', fontWeight:700, color:'#0E0E0C', margin:0 }}>Preguntas <span style={{ color:'#1550E8' }}>frecuentes</span></h3>
          </div>
          <div style={{ background:'#fff', borderRadius:20, overflow:'hidden', border:'1.5px solid rgba(0,0,0,0.06)', boxShadow:'0 4px 20px rgba(0,0,0,0.04)' }}>
            {FAQS.map((faq,i)=>(
              <div key={i} style={{ borderBottom: i<FAQS.length-1 ? '1px solid rgba(0,0,0,0.05)' : 'none' }}>
                <button onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{ width:'100%', background:'none', border:'none', padding:'18px 20px', display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer', textAlign:'left', gap:12 }} data-cursor="pointer">
                ...
                <div style={{ maxHeight:openFaq===i?200:0, overflow:'hidden', transition:'max-height 0.4s cubic-bezier(.23,1,.32,1)' }}>
                  <p style={{ fontFamily:'DM Sans', fontSize:14, color:'#6B6560', lineHeight:1.75, margin:0, padding:'0 20px 18px' }}>{faq.a}</p>
                </div>
              </div>
            ))}
          </div>
